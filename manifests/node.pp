...
class openshift_origin::node {
  ...
  if $::openshift_origin::container_model == 'libvirt' {
    ...
    file { 'libvirt container config':
      ensure  => present,
      path    => '/etc/openshift/node-plugins.d/openshift-origin-container-libvirt.conf',
      content => template('openshift_origin/node/plugins/openshift-origin-container-libvirt.conf.erb'),
      require => Package['rubygem-openshift-origin-container-libvirt'],
      owner   => 'root',
      group   => 'root',
      mode    => '0644',
    }
  ...
  if $::openshift_origin::container_model == 'selinux' {
    ...
    file { 'selinux container config':
      ...
      content => template('openshift_origin/node/plugins/openshift-origin-container-selinux.conf.erb'),
